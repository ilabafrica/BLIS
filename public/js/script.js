...
/* Function to display analyser without page reload */
public function reload()
{
	$('#reloadable').html("");                            
    var id = $('#analyser_id').attr('value');
    var URL_ROOT = 'http://127.0.0.1/lis/public/';
    _token: JSON.stringify($('input[name=_token]').val());
    $.ajax({
        dataType: 'json',
        type: 'POST',
        url:  URL_ROOT+'ajax/analyser',
        data: {analyser_id: id, '_token': $('input[name=_token]').val()},
        success: function(data){
            var select_list = $('#test_type_id');
            select_list.empty();
            select_list.append("<option value=''>Select Test Type</option>");

	...
	<div class="row">
       <div class="col-md-4">
        ...
        <div class="col-md-4">
	    	<div class="row">
		        <div class="col-md-4">
		        	{{ Form::label('report_type', trans_choice('messages.report-type',1)) }}
		        </div>
		        <div class="col-md-8">
			...
			<table class="table table-striped table-hover table-condensed search-table">
				@if($selectedReport==0) <!-- Summary Report-->
					<thead>
						<tr>
							...
							<th>{{trans_choice('messages.received-tests',1)}}</th>
							<th>{{trans_choice('messages.accepted-specimen',1)}}</th>
							<th>{{trans_choice('messages.rejected-specimen',1)}}</th>
							<th>{{trans_choice('messages.performed-tests',1)}}</th>
							<th>{{trans_choice('messages.verified-tests',1)}}</th>
						</tr>
					</thead>
					<tbody>
						<?php $i = 1;?>
						...
						@empty
							<tr>
								<td>{{trans_choice('messages.no-data-found',1)}}</td>
							</tr>
						@endforelse
					</tbody>
				...
				@elseif($selectedReport == 1) <!-- Patients Registry Report-->
					<thead>
						<tr>
							<th></th>
							<th>{{trans_choice('messages.patient-number',1)}}</th>
							<th>{{trans_choice('messages.name',1)}}</th>
							<th>{{trans_choice('messages.gender',1)}}</th>
							<th>{{trans_choice('messages.age',1)}}</th>
							<th>{{trans_choice('messages.registration-date',1)}}</th>
						</tr>
					</thead>
					<tbody>
						<?php $i = 1;?>
						...
						@empty
							<tr>
								<td colspan='6'>{{trans_choice('messages.no-data-found',1)}}</td>
							</tr>
						@endforelse
					</tbody>
				...
				@elseif($selectedReport == 2) <!-- Specimens Registry Report-->
					<thead>
						<tr>
							<th>{{trans_choice('messages.specimen-number',1)}}</th>
				...
				@elseif($selectedReport == 3 || $selectedReport == 4) <!-- Tests Registry Report-->
					<thead>
						<tr>
							<th>{{trans_choice('messages.test-id',1)}}</th>
							<th>{{trans_choice('messages.test-type',1)}}</th>
							<th>{{trans_choice('messages.patient-number',1)}}</th>
							<th>{{trans_choice('messages.patient',1)}}</th>
							<th>{{trans_choice('messages.specimen-id',1)}}</th>
							<th>{{trans_choice('messages.registration-date',1)}}</th>
						</tr>
					</thead>
					<tbody>

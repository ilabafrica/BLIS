...
class VictimRequest extends Request {
	/**
	...
	*/
	public function rules() {
		$id = $this->ingnoreId();
		return [ 
			'incident_date' => 'required:victims,incident_date,'.$id, 
			'scene' => 'required:victims,scene,'.$id, 
			'victim' => 'required:victims,victim,'.$id, 
			'incident_type' => 'required:victims,incident_type,'.$id, 
			'equipment_code' => 'required:victims,equipment_code,'.$id, 
			'sop' => 'required:victims,sop,'.$id,
			'description' => 'required:victims,description,'.$id,
			'first_aid' => 'required:victims,first_aid,'.$id,
			'corrective_action' => 'required:victims,corrective_action,'.$id,
			'reporting_officer' => 'required:victims,reporting_officer,'.$id,
		];
	}
	/**
